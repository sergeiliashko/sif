...
try:
    ...
    script, params_file, vcoords_file = argv
except:
...
import pathminimizer_module as pm

m = 100
dt = np.array([bb-aa])/(m-1)
path = np.array([aa])
for i in range(m-1):
    path = np.vstack((path,(path[i,:]+dt)))
path = (path % (2*pi)).T
en,gr = em.calculateEnergyAndGradient_func(path,M,anisotropy_angles,K,0.0,0.0,V,distances,distance_unit_vectors,factor)
pm.find_mep_func(
        distances,
