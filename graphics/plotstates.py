...
import sif.paramsfactory.geometry as g

output_path = ""#"data/"
output_path_img = ""#"img/"
def main(vcoords_file,energy_path,path,fName="mepstates"):
    ...
    maxm = argrelextrema(energy_path,np.greater)
    pt_min = ((path[:,minm])[:,0]).T
    pt_max = ((path[:,maxm])[:,0]).T
    ...
    plt.savefig(f"{fName}-min.pdf",bbox_inches='tight', dpi=300)
    plt.clf()
    ...
    for i in range(m):
        ...
        #ax.text(0.5, 0.5, rn[i], horizontalalignment='center', verticalalignment='center', transform = ax.transAxes)
        ax.text(0.5, 0.5, '', horizontalalignment='center', verticalalignment='center', transform = ax.transAxes)
    plt.savefig(f"{fName}-max.pdf",bbox_inches='tight', dpi=300)
    plt.clf()
...
if __name__ == "__main__":
    try:
        ...
        energy_path = np.loadtxt(argv[2])
        path = np.loadtxt(argv[3])
        main(argv[1], energy_path, path)
